...
        uploaded_file_obj = None # To keep track of the file object if uploaded
...
            uploaded_file_obj = tk.request.files['background_image_upload']
            # Dosyayı kaydet ve yolunu al
            upload.upload(uploaded_file_obj) # uploader.get_max_image_size() KALDIRILDI
...
                full_path_to_clear = os.path.join(ckan.common.config.get('ckan.storage_path'), 'storage', 'uploads', 'theme_background', current_background_image_path)
                if os.path.exists(full_path_to_clear):
                    os.remove(full_path_to_clear) # Eski dosyayı sil
...
                full_path_to_clear = os.path.join(ckan.common.config.get('ckan.storage_path'), 'storage', 'uploads', 'theme_background', current_background_image_path)
                if os.path.exists(full_path_to_clear):
                    os.remove(full_path_to_clear)
            # Yeni dosyayı kaydet ve yolunu al
            upload.upload(uploaded_file_obj) 
...
            full_path_to_clear = os.path.join(ckan.common.config.get('ckan.storage_path'), 'storage', 'uploads', 'theme_background', background_image_path)
            if os.path.exists(full_path_to_clear):
                os.remove(full_path_to_clear)
